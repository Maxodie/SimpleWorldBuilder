...
struct VertexBufferElement {
  ...
  VertexBufferElement(std::string name, ShaderElementType shaderType,
                      bool isNormalized = false)
      : Size(ConvertShaderTypeToSize(shaderType)), IsNormalized(isNormalized),
        Offset(0), Type(shaderType), Name(name) {}
...
public:
  ...
  WB_INLINE const std::vector<VertexBufferElement> &GetElements() {
    return m_elements;
  }
...
struct Vertex3D {
  Vertex3D(glm::vec4 color, glm::vec3 vec, glm::vec3 scale)
      : Color(color), Position(vec), Scale(scale) {}

  Vertex3D(float x, float y, float z) : Position(x, y, z) {}
...
template <typename TVertex> class VertexArrayBuffer {
public:
  VertexArrayBuffer() = default;

  virtual void
  AddVertexBuffer(SharedPtr<VertexBuffer<TVertex>> &vertexBuffer) = 0;
  WB_INLINE virtual void Bind() = 0;

  [[nodiscard]] WB_INLINE std::vector<SharedPtr<VertexBuffer<TVertex>>> &
  GetLayout() {
    return m_layout;
  }
